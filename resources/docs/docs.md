...
The `home-routes` handler in the `re-pipe.routes.home` namespace
...
The `home-page` function will in turn call the `re-pipe.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/re_pipe/core.cljs` file.



<p class="title is-5">Organizing the routes</p>

The routes are aggregated and wrapped with middleware in the `re-pipe.handler` namespace:
...
Request middleware functions are located under the `re-pipe.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `re-pipe.dev-middleware` namespace found in
